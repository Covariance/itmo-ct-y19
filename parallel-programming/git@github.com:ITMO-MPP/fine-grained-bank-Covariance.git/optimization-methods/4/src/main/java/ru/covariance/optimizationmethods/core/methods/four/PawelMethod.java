...
public class PawelMethod extends AbstractMultidimMinimizer {
    Matrix G;
    Point x1;
    double alpha;
    Point s;
    ...
    public PawelMethod(ToDoubleFunction<Point> f, UnaryOperator<Point> grad, Function<Point, Matrix> gessian, Point x0) {
...
//        System.out.println("номер итерации & $|s_k|$ & $x_k$ \\\\");
    ...
    public void iterate() {
        ...
        s = p.multiply(alpha);
        Point dx = p.multiply(alpha);
        x1 = x;
        x = x.add(dx);
        Point w = grad.apply(x).subtract(grad.apply(x1));
        Point dx_ = dx.add(G.multiply(w));
        G = G.sub(dx_.mul(dx_).mul(1 / w.dotProduct(dx_)));
    }
